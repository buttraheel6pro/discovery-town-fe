/** Public rentals products API — fetches rental inventory by category. */

import { apiClient, isApiEnabled } from '@/lib/api/client'
import type { PaginatedResponse } from '@/lib/api/client'

export interface RentalPublicProduct {
  id: string
  name: string
...
export interface RentalProductsPage {
  products: RentalPublicProduct[]
  hasMore: boolean
  page: number
}

export interface RentalPublicCategory {
  id: string
  name: string
...
export async function fetchRentalCategories(): Promise<RentalPublicCategory[]> {
  if (!isApiEnabled) return []

  const tenantId = process.env.NEXT_PUBLIC_TENANT_ID
  if (!tenantId) return []

  const params = new URLSearchParams({ tenantId, catalogSlug: 'rentals' })

  const res = await apiClient.get<RentalPublicCategory[]>(
...
export async function fetchRentalProductsByCategory(
  categoryId: string,
  page: number,
  limit: number,
): Promise<RentalProductsPage> {
  ...
  const products: RentalPublicProduct[] = items.map((item) => ({
    ...
    fulfillment: (item.fulfillment as string | null) ?? null,
    requiresDelivery: (item.requiresDelivery as boolean) ?? false,
    requiresStaff: (item.requiresStaff as boolean) ?? false,
