/** Public shop products API — fetches inventory products by shop category. */
...
export interface ShopPublicCategory {
  id: string
  name: string
...
export async function fetchShopCategories(): Promise<ShopPublicCategory[]> {
  if (!isApiEnabled) return []

  const tenantId = process.env.NEXT_PUBLIC_TENANT_ID
  if (!tenantId) return []

  const params = new URLSearchParams({ tenantId, catalogSlug: 'shop' })

  const res = await apiClient.get<ShopPublicCategory[]>(
...
export async function fetchShopProductsByCategory(
  ...
  const products: ShopPublicProduct[] = items.map((item) => ({
